...
# ----------------------------- LIVE SMA DASHBOARD -----------------------------
    ...
    short_window = st.number_input("Short SMA Window (minutes)", min_value=1, value=5)
    long_window = st.number_input("Long SMA Window (minutes)", min_value=1, value=15)
    ...
    def fetch_data(ticker, short_window, long_window):
        ...
        if df.empty:
            return pd.DataFrame()
        short_window_adj = min(short_window, len(df))
        long_window_adj = min(long_window, len(df))
        df['SMA_short'] = df['Close'].rolling(window=short_window_adj).mean()
        df['SMA_long'] = df['Close'].rolling(window=long_window_adj).mean()
    ...
    if df.empty or len(df) < 2:
        ...
        st.warning("Not enough data to calculate SMAs or signals.")
    else:
        latest = df.iloc[-1]
        prev = df.iloc[-2]
        signal = "HOLD"
    ...
    if not df.empty:
        ...
        with col1:
            ...
            st.metric(f"Short SMA ({short_window}m)", f"${df['SMA_short'].iloc[-1]:.2f}")
        with col2:
            st.metric(f"Long SMA ({long_window}m)", f"${df['SMA_long'].iloc[-1]:.2f}")
        ...
        fig.add_trace(go.Scatter(x=df.index, y=df['SMA_long'], mode='lines', name=f"SMA {long_window}"))
        fig.update_layout(title=f"{ticker} Live Chart", xaxis_rangeslider_visible=False, height=600)
